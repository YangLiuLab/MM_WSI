...
for j = 1:nb_horizontal_tiles
  ...
  for i = 1:nb_vertical_tiles
    ...
    if i > 1
	  ...
	  M1 = I1 - mean(I1(:)); M1(M1<0)=0; M1 = M1/max(M1(:)); M1 = imadjust(M1);
	  M2 = I2 - mean(I2(:)); M2(M2<0)=0; M2 = M2/max(M2(:)); M2 = imadjust(M2);
      [t_Y1(i,j), t_X1(i,j), CC1(i,j)] = pciam(M2, M1, StitchingConstants.NORTH, StitchingConstants.NB_FFT_PEAKS, 0);
    ...
    if j > 1
	  ...
	  M1 = I1 - mean(I1(:)); M1(M1<0)=0; M1 = M1/max(M1(:)); M1 = imadjust(M1);
	  M2 = I2 - mean(I2(:)); M2(M2<0)=0; M2 = M2/max(M2(:)); M2 = imadjust(M2);
      [t_Y2(i,j), t_X2(i,j), CC2(i,j)] = pciam(M2, M1, StitchingConstants.WEST, StitchingConstants.NB_FFT_PEAKS, 0);
    end
  end
end
